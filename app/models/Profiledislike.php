...
class Profiledislike{
	...
	public function disLikeProfile($profile_id){
		...
		$this->db->query("SELECT dislike_id FROM profile_dislikes WHERE user_id = :current_user_id AND profile_id = :profile_id AND disliked = 0");
		$this->db->bind(':current_user_id', $_SESSION['user_id']);
		$this->db->bind(':profile_id', $profile_id);
		$this->db->execute();
		...
		if($count === 1){
			...
			$this->db->query("UPDATE profile_dislikes SET disliked = 1 WHERE user_id = :current_user_id AND profile_id = :profile_id");
			$this->db->bind(':current_user_id', $_SESSION['user_id']);
			$this->db->bind(':profile_id', $profile_id);
			$this->db->execute();
			$this->update_profile_dislikes_count($profile_id);
			...
			$this->db->query("INSERT INTO profile_dislikes(user_id,profile_id,disliked) VALUES(:current_user_id,:profile_id,1)");
			$this->db->bind(':current_user_id', $_SESSION['user_id']);
			$this->db->bind(':profile_id', $profile_id);
			$this->db->execute();
			$this->update_profile_dislikes_count($profile_id);
	...
	public function dislikedprofile($profile_id){
		$this->db->query("SELECT dislike_id FROM profile_dislikes WHERE profile_id = :profile_id AND user_id = :current_user_id AND disliked = 1");
		$this->db->bind(':current_user_id', $_SESSION['user_id']);
		$this->db->bind(':profile_id', $profile_id);
		$this->db->execute();
		$count = $this->db->rowCount();
