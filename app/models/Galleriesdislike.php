...
class Galleriesdislike{
	...
	public function dislike_gallery($gallery_id){
		...
		$this->db->query("SELECT dislike_id FROM galleries_dislikes WHERE user_id = :current_user_id AND gallery_id = :gallery_id AND disliked = 0");
		$this->db->bind(':current_user_id', $_SESSION['user_id']);
		$this->db->bind(':gallery_id', $gallery_id);
		$this->db->execute();
		...
		if($count === 1){
			...
			$this->db->query("UPDATE galleries_dislikes SET disliked = 1 WHERE user_id = :current_user_id AND gallery_id = :gallery_id");
			$this->db->bind(':current_user_id', $_SESSION['user_id']);
			$this->db->bind(':gallery_id', $gallery_id);
			$this->db->execute();
			$this->update_gallery_dislikes_count($gallery_id);
			...
			$this->db->query("INSERT INTO galleries_dislikes(user_id,gallery_id,disliked) VALUES(:current_user_id,:gallery_id,1)");
			$this->db->bind(':current_user_id', $_SESSION['user_id']);
			$this->db->bind(':gallery_id', $gallery_id);
			$this->db->execute();
			$this->update_gallery_dislikes_count($gallery_id);
	...
	public function disliked_gallery($gallery_id){
		$this->db->query("SELECT dislike_id FROM galleries_dislikes WHERE gallery_id = :gallery_id AND user_id = :current_user_id AND disliked = 1");
		$this->db->bind(':current_user_id', $_SESSION['user_id']);
		$this->db->bind(':gallery_id', $gallery_id);
		$this->db->execute();
		$count = $this->db->rowCount();
	...
	public function new_galleries_dislikes_count($gallery_id){
	...
	public function update_gallery_dislikes_count($gallery_id){
		$new_galleries_dislikes_count = $this->new_galleries_dislikes_count($gallery_id);
		$this->db->query("UPDATE photos_galleries SET dislikes_count = :dislikes_count WHERE gallery_id = :gallery_id");
		$this->db->bind(':dislikes_count', $new_galleries_dislikes_count);
		$this->db->bind(':gallery_id', $gallery_id);
		return $this->db->execute();
	}
	public function new_galleries_likes_count($gallery_id){
	...
	public function update_gallery_likes_count($gallery_id){
		$new_galleries_likes_count = $this->new_galleries_likes_count($gallery_id);
		$this->db->query("UPDATE photos_galleries SET likes_count = :likes_count WHERE gallery_id = :gallery_id");
		$this->db->bind(':likes_count', $new_galleries_likes_count);
		$this->db->bind(':gallery_id', $gallery_id);
		return $this->db->execute();
	}
}